 ...
 *--------------------------------------------------------------------------------*/		 
	...
	insert(&newBST, &VH, prod02);
	insert(&newBST, &VH, prod03);
	insert(&newBST, &VH, prod04);
 ...
 ************************************************************/
 void insert(BST *B, VHeap *VH, product P)
 {
	  ...
	  if(VH->avail == -1) {
			...
			for(; *B != -1 && strcmp(VH->VH_node[*B].elem.prodID, P.prodID) != 0;){
			...
			if(*B == -1) {
					...
					VH->VH_node[index].LC = -1;
					VH->VH_node[index].RC = -1;
					*B = index;
